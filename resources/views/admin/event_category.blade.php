                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="col-sm-12" style="text-align: right; margin-top:10px;">
                                <a href="{{ route('admin.add_event_category') }}"><button
...
<script>
    ...
    serverSide: true,
    ajax: {
        url: "{{ route('admin.event_category_list') }}",
        type: "GET"
    },
    columns: [
        { data: "no" },
...
function makeRowsSortable() {
    $("#example1 tbody").sortable({
        ...
        update: function(event, ui) {
            ...
            $.ajax({
                url: "{{ route('admin.event_category_reorder') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    order: order
...
function changestatus(id) {
    ...
    $.ajax({
        url: "{{ route('admin.event_category_status_change') }}",
        type: "POST",
        data: {
            _token: csrfToken,
            id: id,
...
function deleteuser(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let csrfToken = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: "{{ route('admin.event_category_delete') }}",
                type: "POST",
                data: {
                    _token: csrfToken,
                    id: id,

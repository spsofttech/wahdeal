...
@section('title', 'Booking Cancel Reason')
    ...
    <div class="container-fluid">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Booking Cancel Reason</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
                            <li class="breadcrumb-item active">Booking Cancel Reason</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="col-sm-12" style="text-align: right; margin-top:10px;">
                                <a href="{{ route('admin.add_booking_cancel_reason') }}"><button
...
<script>
    ...
    $('#example1').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "{{ route('admin.booking_cancel_reason_list') }}", // Your AJAX URL
            "type": "GET",
            "data": function(d) {
...
function changestatus(id) {
    ...
    $.ajax({
        url: "{{ route('admin.booking_cancel_reason_status_change') }}",
        type: "POST",
        data: {
            _token: csrfToken,
            id: id,
...
function deleteuser(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let csrfToken = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: "{{ route('admin.booking_cancel_reason_delete') }}",
                type: "POST",
                data: {
                    _token: csrfToken,
                    id: id,

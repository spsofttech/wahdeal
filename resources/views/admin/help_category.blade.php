                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="col-sm-12" style="text-align: right; margin-top:10px;">
                                <a href="{{ route('admin.add_help_category') }}"><button
...
$(document).ready(function() {
    $('#example1').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "{{ route('admin.help_category_list') }}", // Your AJAX URL
            "type": "GET",
            "data": function(d) {
...
function changestatus(id) {
    ...
    $.ajax({
        url: "{{ route('admin.help_category_status_change') }}",
        type: "POST",
        data: {
            _token: csrfToken,
            id: id,
...
function deleteuser(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let csrfToken = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: "{{ route('admin.help_category_delete') }}",
                type: "POST",
                data: {
                    _token: csrfToken,
                    id: id,

                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="col-sm-12" style="text-align: right; margin-top:10px;">
                                <a href="{{ route('admin.add_offer_type') }}"><button
...
<script>
    ...
    $('#example1').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "{{ route('admin.offer_type_list') }}", // Your AJAX URL
            "type": "GET",
            "data": function(d) {
...
function changestatus(id) {
    ...
    $.ajax({
        url: "{{ route('admin.offer_type_status_change') }}",
        type: "POST",
        data: {
            _token: csrfToken,
            id: id,
...
function deleteuser(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            let csrfToken = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
                url: "{{ route('admin.offer_type_delete') }}",
                type: "POST",
                data: {
                    _token: csrfToken,
                    id: id,
